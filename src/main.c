...
int main(void)
{
    ...
    while(!glfwWindowShouldClose(window)) {
        ...
        loc = shader_get_uniform_location(ren, shader, "u_view");
        if(loc < 0) return -1;
        glUniformMatrix4fv(loc, 1, GL_FALSE, view.data);

        loc = shader_get_uniform_location(ren, shader, "u_proj");
        if(loc < 0) return -1;
        glUniformMatrix4fv(loc, 1, GL_FALSE, camera.projection.data);

        for(int i = 0; i < ARRAY_LEN(cube_positions); ++i) {
            Mat4 model = mat4_eye(1.0f);
            model = mat4_translate(model, cube_positions[i]);
            loc = shader_get_uniform_location(ren, shader, "u_model");
            if(loc < 0) return -1;
